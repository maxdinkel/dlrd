...
var_distribution = MeanFieldSinhArcsinhVariational(num_dim)
...
for col, optimizer in enumerate(['adam', 'rmsprop']):
    for i, result_file in enumerate(result_files):
        ...
        with open(output_dir / f"{optimizer}_n16_{result_file}.pickle", 'rb') as handle:
            results = pickle.load(handle)
...
for col in range(ncols):
    ...
    axes[0, col].set_yticks(np.logspace(-3, 2, num=6))
...
plt.savefig(output_dir / 'div_wine.pdf')
